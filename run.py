...
def calculate_surplus_data(sales_row_list):
    """
    ...
    surplus_data = []
    for stock, sales in zip(stock_row, sales_row_list):
...
def update_surplus_worksheet(data):
    """
    Updates the surplus sheet with the calculated surplus values
    """
...
def get_last_5_entries_sales():
    """
    Collects columsn fo data from sales worksheet, collecting
    ...
    for ind in range(1, 7):
        # columns do not begin at index 0 but rather 1
        column = sales.col_values(ind)
        columns.append(column[-5:]) 
        # [-5:] returns only the last five items     
